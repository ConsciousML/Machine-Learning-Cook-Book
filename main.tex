...
\chapter{Numerical Computation}
    ...
    \section{Constrained Optimization}
        ...
        When using the generalized Lagrange function, some requirements must be met known as \textbf{KKT conditions} such as:
        \begin{align}
            &\nabla f(\boldsymbol{x}^*) - \sum_i{\lambda_i \nabla g^{(i)}(\boldsymbol{x}^*)} - \sum_j{\sigma_j \nabla h^{(j)}(\boldsymbol{x}^*)}  = 0  \quad \text{for maximization}\\
            &\nabla f(\boldsymbol{x}^*) + \sum_i{\lambda_i \nabla g^{(i)}(\boldsymbol{x}^*)} + \sum_j{\sigma_j \nabla h^{(j)}(\boldsymbol{x}^*)}  = 0   \quad \text{for minimization}
        \end{align}
        where $\boldsymbol{x}^*$ is a local optimum.
        
        Minimizing the Lagrangian introduces the \textbf{dual form} of the problem as:
        \begin{equation}
            q(\boldsymbol{\lambda}, \boldsymbol{\sigma}) = \min_{\boldsymbol{x}} L(\boldsymbol{x, \boldsymbol{\lambda}, \boldsymbol{\sigma}})
        \end{equation}
        
        Solving the dual problem results in finding a lower bound of the primal problem:
        \begin{equation}
            q(\boldsymbol{\lambda}, \boldsymbol{\sigma}) \leq f(\boldsymbol{x^*})
        \end{equation}
        where $\boldsymbol{\sigma} \geq 0$ and $\boldsymbol{x^*}$ is an optimal solution of the primal problem.
        
        We want to find the best lower bound such as:
        \begin{align}
            &\max_{\boldsymbol{\lambda}, \boldsymbol{\sigma}} q(\boldsymbol{\lambda}, \boldsymbol{\sigma}) \\
            \text{s.t} \quad &\boldsymbol{\sigma} \geq 0 \\
            & (\boldsymbol{\lambda}, \boldsymbol{\sigma}) \in \{\boldsymbol{\lambda}, \boldsymbol{\sigma} | q(\boldsymbol{\lambda}, \boldsymbol{\sigma}) > -\infty\}
        \end{align}
        
        The generalized Lagrangian allows us to incorporate the inequality and equality constrains into the objective function resulting in a optimization problem without constrains known a the dual problem. Because the dual problem is a lower bound of the primal problem, we try to find the maximum lower bound such as:
        \begin{equation}
            q(\boldsymbol{\lambda}^*, \boldsymbol{\sigma}^*) = f(\boldsymbol{x^*})
        \end{equation}
        where $\boldsymbol{\lambda}^*, \boldsymbol{\sigma}^*$ are the solution that maximize $q$.
...
\chapter{Machine Learning Concepts}
    ...
    \subsection{Support Vector Machines}
        \subsubsection{Kernel functions}
            ...
            The \textbf{kernel trick} method stands for calculating the relationship of data in higher dimension without actually transforming the data in this target space. The kernel trick reduces the amount of computation required of kernelized algorithms.
            
            The main idea is, instead of computing $\phi(\boldsymbol{X})$, we will compute the dot product $\phi(\boldsymbol{X}_i)^\top\phi(\boldsymbol{X}_j)$ for all the components of $\boldsymbol{X}$. In fact, we will know the relationship of the data in higher dimension, without ever computing the transformation of the whole data.
            ...
            \end{equation}
            is positive definite for any set of input $\{\boldsymbol{x}_i\}_{i=1}^{n}$. Such a kernel is called a \textbf{Mercer kernel}. The importance of Mercer kernels lies in the \textbf{Mercer's theorem}: if the Gram matrix of a kernel is positive definite, we can compute an eigen vector decomposition such as:
            \begin{equation}
        ...
        \subsubsection{Hard-Margin Support Vector Machines}
            ...
            In this section we will discuss the \textbf{Support vector classifier}.
            ...
            SVMs try to maximize the margin when seperating the data. This fact plays a role in the generalization of SVM models. We can find the value of the margin by computing:
            \begin{align}
            ...
            \end{align}
            In other words, the optimization problem to solve when training an SVM is to:
            \begin{equation}
            ...
            The condition ensures that all samples are on the right side of the hyperplane. Note that this classifier does not allow any misclassification and is known as \textbf{hard margin classifier}.
            
            The minimization above is in its primal form, we can use the generalized Lagrangian to get its dual form such as:
            \begin{equation}
                \min_{\boldsymbol{x}, b}L(\boldsymbol{x}, b, \boldsymbol{\sigma}) = \frac{1}{2}||\boldsymbol{w}||^2 - \sum_{i=1}^{N} \sigma_i y_i(\boldsymbol{w}^\top \boldsymbol{x}_i + b - 1)
                \label{eq:lagrange}
            \end{equation}
            We also need to satisfy the KKT conditions, such as:
            \begin{align}
                \frac{\partial L}{\partial \boldsymbol{w}} = \boldsymbol{w} - \sum_{i=1}^{N} \sigma_i y_i\boldsymbol{x}_i &= 0 \Rightarrow \boldsymbol{w} = \sum_{i=1}^{N} \sigma_i y_i\boldsymbol{x}_i \\
            ...
            \begin{equation}
                \max_{\boldsymbol{\sigma}}\min_{\boldsymbol{x}, b}L(\boldsymbol{x}, b, \boldsymbol{\sigma})
            \end{equation}
            By substituting these values in \ref{eq:lagrange} we get:
            \begin{equation}
                \min_{\boldsymbol{x}, b}L(\boldsymbol{x}, b, \boldsymbol{\sigma}) = \sum_{i=1}^{N}\sigma_i - \frac{1}{2} \sum_{i=1}^{N}\sum_{j=1}^{N} \sigma_i \sigma_j y_i y_j \boldsymbol{x}_i^\top \boldsymbol{x}_j
            \end{equation}
            
            The final dual optimization is:
            \begin{align}
                ...
                \text{s.t.} \quad y_i(\boldsymbol{w}^\top \boldsymbol{x}) + b \geq 1
            \end{align}
            
            The greatest quality of support vector machines is that they never get stuck in a local minima, respectively maxima if we have a maximization problem.
            
            For predicting on new samples, we use:
            \begin{equation}
                y^{\text{test}} = \text{sign}\left(\sum_{i=1}^{N}\boldsymbol{w}^\top \boldsymbol{x}^{\text{test}} + b\right)
            \end{equation}

            If the data is non linearly separable, we can use the kernel trick discussed in the previous section to translate the data into a higher dimensional space where it is linearly separable. It is quite important to remember that the kernel trick allows us to avoid computing the transformation in the higher dimensional space which saves a lot of computational time.
            
            By introducing the kernel trick, the optimization problem is such as:
            \begin{align}
                \max_{\boldsymbol{\sigma}; \boldsymbol{\sigma} \geq 0}\sum_{i=1}^{N}\sigma_i - \frac{1}{2} \sum_{i=1}^{N}\sum_{j=1}^{N} \sigma_i \sigma_j y_i y_j \phi(\boldsymbol{x}_i)^\top \phi(\boldsymbol{x}_j) \\
                \text{s.t.} \quad y_i(\boldsymbol{w}^\top \phi(\boldsymbol{x})) + b \geq 1
            \end{align}
            and the prediction:
            \begin{equation}
                y^{\text{test}} = \text{sign}\left(\sum_{i=1}^{N}\boldsymbol{w}^\top \phi(\boldsymbol{x}^{\text{test}}) + b\right)
            \end{equation}

            
        \subsubsection{Soft-Margin Support Vector Machines}

            If the data is not linearly separable, even after using the kernel trick, we can use a \textbf{soft margin classifier}. We introduce slack variables $\zeta_i \geq 0$ where:
            \begin{itemize}
                \item $\zeta_i = 0$ means that the point lies in the correct margin boundary and $\zeta_i = |y_i - f_i|$ otherwise.
                \item $0 < \zeta_i \leq 1$ the point lies inside the margin and on the correct side.
                \item $\zeta_i > 1$ the point lies in the incorrect boundary.
            \end{itemize}
            We replace the hard margin constraint by:
            \begin{equation}
                y_i(\boldsymbol{w}^\top \phi(\boldsymbol{x})) + b \geq 1 - \zeta_i     
            \end{equation}
            ...
            \begin{align}
                ...
                &\zeta_i \geq 0 && \forall i \in \{1, \dots, N\}
            \end{align}
            The soft margin penalization amkes the model be more flexible and allows for some misclassification just like so:
            \ref{fig:svm}.
            \begin{figure}[ht]
                \centering
                \includegraphics[width=0.8\textwidth]{softmargin.png}
                \caption{Discrimination schema of a soft-margin SVM}
                \label{fig:svm}
            \end{figure}
